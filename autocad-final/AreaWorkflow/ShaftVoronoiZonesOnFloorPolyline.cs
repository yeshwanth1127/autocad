                ...
                for (int zi = 0; zi < zoneRings.Count; zi++)
                {
                    ...
                    if (ring == null || ring.Count < 3)
                    {
                        ed.WriteMessage(
                            ...
                            " skipped: invalid ring (" + (ring?.Count ?? 0).ToString(CultureInfo.InvariantCulture) + " vertices).\n");
                        // Preserve index alignment so ownerIndexPerRing[zi] still matches createdZonePolylineHandles[zi].
                        createdZonePolylineHandles?.Add(null);
                        continue;
                    }
